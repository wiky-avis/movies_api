...
class GetMoviesList(Action):
    ...
    def __call__(self) -> List[ShortMovie]:
        ...
        request_data = {
            ...
            "from": (self.resources.search.page - 1)
            * self.resources.search.limit,
            "sort": [{sort_value: self.resources.search.sort_order.value}],
        ...
        data = self.resources.es_client.search(
            body=request_data, index="movies"
        )
